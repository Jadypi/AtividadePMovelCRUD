...
export function ManterProdutos({ navigation, route }) {
    ...
    const [nomeProdut, setNomeProdut] = useState(route?.params?.nomeProdut)
    ...
    async function salvarProduto() {
        if (nomeProduto == '') {
            ...
            let resultado = ''
            if (route?.params) {
                resultado = await atualizarProdutos(route?.params?.id, { nomeProduto, precoProduto, nomeProdut })
            }
            else {
                resultado = await cadastrarProdutos({ nomeProduto, precoProduto, nomeProdut })
            }
            if (resultado == 'erro') {
    ...
    return (
        <View style={estilos.container}>
            <TextInput
                ...
                mode="outlined"
                error={statusErro == 'Atividade'}
                style={estilos.input} />
            {statusErro == 'Atividade' ? <HelperText type="error" visible={statusErro == 'Atividade'}>
                {mensagemErro}
            </HelperText> : null}
